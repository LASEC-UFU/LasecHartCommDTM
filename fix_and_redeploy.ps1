# Same as fix_fdt_registry.ps1 but writes output to a file
$logFile = "C:\SourceCode\LasecHartCommDTM\fix_output.log"
...
"@
    ...
    if ($hr -eq 0) {
        ...
        Write-Host "   FAILED: HRESULT 0x$($hr.ToString('X8'))"
    }
} else {
    Write-Host "   FDT100.dll NOT FOUND"
}
...
if (Test-Path $cwOcx) {
    & regsvr32 /s $cwOcx
    Write-Host "   regsvr32 done (exit=$LASTEXITCODE)"
} else {
    Write-Host "   CWHARTFDT.ocx NOT FOUND"
}

Write-Host ""
Write-Host "3. Verifying FDT interfaces..."
...
Write-Host ""
Write-Host "4. Now re-registering our DTM WITHOUT /tlb..."
...
if (Test-Path $dtmDll) {
    ...
    Write-Host "   regasm /codebase done"
} else {
    Write-Host "   DLL not found - run build first"
}
